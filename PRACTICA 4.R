...
GB1<-ggplot(penguins, aes(x=año))+
  geom_bar(colour= "chartreuse", fill=color)+
...
HG<-ggplot(penguins, aes(x=largo_aleta_mm))+
  geom_histogram(col="darkolivegreen1", fill="darkorchid")+
  ggtitle("Histograma")+
...
GD<-ggplot(penguins, aes(x=largo_pico_mm, y=grosor_pico_mm))+
  ...
  scale_color_manual(values=c('blue','gold',"cyan"))
...
GD<-ggplot(penguins, aes(x=genero, y=especie))+
  geom_point(aes(color=especie))+
  scale_color_manual(values=c('blue','gold',"cyan"))
  ggtitle("Gráfico de dispersión")+
  xlab("largo del pico (mm)")+
...
df<-read.csv("penguins")

ggplot(df,aes(x="largo del pico (mm)",y="grosor del pico (mm)"))+
